...
- `module`: (**可选**) 指定模块名，用于在多模块项目根目录运行插件的场景，默认值：`" "`
- `prune`: (**可选**) 开启或关闭迭代生成测试时剔除出现断言错误的测试用例，默认值：`false`
...
以下是单个模块的插件生成指南，如果您希望在多模块项目下正确使用插件，需要指定`module`参数 或者 进入子模块根目录（pom所在目录）运行插件。
